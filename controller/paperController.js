...
exports.createPaper = async (req, res) => {
  try {
    ...
    if (paper === null) {
      ...
      res
        .status(404)
        .json({ status: "fail", message: "创建失败，试卷批次重复!" });
    }
  } catch (err) {
    res.status(404).json({ status: "fail", message: err });
  }
...
exports.createResitPaper = async (req, res) => {
  try {
    const paper = await Paper.findOne(
      {
        paper_batch: req.body.paper_batch,
    ...
    for (var i = 0; i < paper_amount; i++) {
      if (
        ...
        res.status(200).json({
          status: "false",
          message:
            "已经存在名称批次年份相同的试卷！两个试卷的名称批次年份不能完全相同！",
        });
      }
    }
    if (isRepeat == 0) {
...
exports.deletePaper = async (req, res) => {
  try {
    ...
    if (Date.now() - paperInfo.start_time < 0) {
      ...
      const readyToDeleteUserpapers = await UserPaper.deleteMany({
      ...
      res
        .status(204)
        .json({ status: true, message: "cann't delete this paper!" });
    }
  } catch (err) {
    res.status(404).json({ status: "fail", message: err });
  }
    ...
    if (data1 != null) {
      ...
      if (
        ...
        for (var i = 0; i < grade.length; i++) {
          grade[i] =
            grade[i].public_score +
            grade[i].subpublic_score +
            grade[i].professional_score;
          if (grade[i] >= 0 && grade[i] <= 10) {
            grades[0]++;
          } // 0-10分段
          else if (grade[i] >= 11 && grade[i] < 21) {
            grades[1]++;
          } // 11-20分段
          else if (grade[i] >= 21 && grade[i] < 31) {
            grades[2]++;
          } // 21-30分段
          else if (grade[i] >= 31 && grade[i] < 41) {
            grades[3]++;
          } // 31-40
          else if (grade[i] >= 41 && grade[i] < 51) {
            grades[4]++;
          } // 41-50
          else if (grade[i] >= 51 && grade[i] < 61) {
            grades[5]++;
          } // 51-60
          else if (grade[i] >= 61 && grade[i] < 71) {
            grades[6]++;
          } // 61-70
          else if (grade[i] >= 71 && grade[i] < 81) {
            grades[7]++;
          } // 71-80
          else if (grade[i] >= 81 && grade[i] < 91) {
            grades[8]++;
          } // 81-90
          else {
            grades[9]++;
